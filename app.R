      ...
      tabsetPanel(
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     ...
                     sliderInput("max_count","Max # of countries",
                                 min=5,max=20,value=9,step=1),
                     radioButtons("sorting","Top countries based on", c("Cases"="Cases",
                                                                        "Deaths"="Deaths",
                                                                        "Deaths/Cases"="Deaths_per_Cases",
                                                                        "Population (in Millions)"="Popn_in_Millions")),
                     hr(),
                     ...
                     h6(tags$li("https://raw.githubusercontent.com/albertyw/avenews/
                                master/old/data/average-latitude-longitude-countries.csv")),
                   ...
                   mainPanel(
                     fluidRow(
                       ...
                       column(width=12,h3(tags$b(textOutput("title1_text"))),
                         style = "background-color:#eeaa88",align="right")
                     ),
                    
                     fluidRow(
                       
                       column(width=3,img(src='coronavirus.jpg',height=300,width=350)),
                       column(width=4,tableOutput("world_Count_total"),offset=2)
                       ),
                     
                     fluidRow(
                       column(width=12,h3(tags$b(textOutput("title2_text"))),
                              style = "background-color:#eeaa88",align="center")
                     ),
                     
                     fluidRow(
                       column(width=12,plotOutput("time_series2")),
                       column(width=12,plotOutput("time_series1"))
                     )
        ...
        tabPanel(title=h3(tags$b("COVID Map")),
                 mainPanel(
                   ...
                   fluidRow(
                     column(width=12,selectInput("country",h4(tags$b("Click on the white bar to select one or more countries.
                                                                     Hover on the marker to view their data.
                                                                     Click on the Marker to view name of the country.
                                                                     The heatmap shows total number of Cases per Million population.")),
                                                choices=country_list,
                                                multiple=TRUE,width=800
                                                
                     ),style = "background-color:#bbaa88;",align="center"
                     ),
                     column(width=8,leafletOutput("leafletmap", height = "600px", width = "860px"))                                                                 
...
server <- function(input,output) {
  ...
  output$world_Count_total <- renderTable({
      ...
      mutate("Deaths/Cases"=Deaths/Cases) %>% arrange(desc(switch(input$sorting, Cases=Cases, 
                           Deaths=Deaths,
  ...
  output$leafletmap <- renderLeaflet({
    ...
    leaflet(data=global) %>%
      ...
      fitBounds(min(global$Long), min(global$Lat),
                max(global$Long), max(global$Lat))%>% addTiles() %>%
      
      addHeatmap(lng=global$Long,lat=global$Lat,
                 intensity=global$cases/1000000,max=1,radius=25,blur=50) 
  })
